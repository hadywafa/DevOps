...
- **Usage**:
  - Signs certificates for the API server, kubelets, etcd, and other components.
...
- **Purpose**: Secures communication between the API server and other components like `kubectl` and kubelets.
...
- **API Server**: Uses its certificate (`apiserver.crt`) to secure communication with clients (`kubectl`, kubelets) and to securely connect to etcd using the `apiserver-etcd-client.crt`.
