...
You can set up a separate **VM to act as a load balancer** for your Kubernetes nodes (with IPs `192.168.1.51` and `192.168.1.52`). This approach allows you to have a more traditional load balancing setup, distributing traffic across the worker nodes while keeping your master node for control-plane tasks.
...
## Steps to Set Up a Separate VM as a Load Balancer for a Kubernetes Cluster

This guide walks through setting up a separate VM as a load balancer for your Kubernetes worker nodes using **NGINX**. You will configure the load balancer to route HTTP and HTTPS traffic to your worker nodes and test the setup using a sample application, `myapp`.
