...
sudo ip netns exec $NODE1_NS1 ip route add default via $NODE1_BR_IP dev veth11
# if any packet in NS2 needs to go to bridge it should talk veth21 by default
sudo ip netns exec $NODE1_NS2 ip route add default via $NODE1_BR_IP dev veth21
```

#### Node 2

```bash
sudo ip netns exec $NODE2_NS1 ip route add $NODE2_BR_IP via dev veth11
sudo ip netns exec $NODE2_NS2 ip route add $NODE2_BR_IP via dev veth21
```
...
sudo iptables -A FORWARD -i br0 -m state --state RELATED,ESTABLISHED -j ACCEPT
...
<!-- ```bash
sudo ip netns exec $NODE1_NS1 iptables -t nat -A POSTROUTING -o br0 -j MASQUERADE
sudo ip netns exec $NODE1_NS2 iptables -t nat -A POSTROUTING -o br0 -j MASQUERADE
``` -->
...
# check the interfaces in the bridge
sudo ip a
...
sudo ip netns exec $NODE1_NS1 ping -c 3 $NODE1_BR_IP
...
sudo ip netns exec $NODE1_NS1 ping -c 3 $NODE2_BR_IP
